...
1. **识别 slug**：如果用户提供的是题号（如 "1"），转换为 slug（如 "two-sum"）——可通过 leetcode.cn 题目列表或常见映射表
2. **运行抓取脚本**：执行 `python3 skills/algorithm-visualization/scripts/fetch-leetcode-problem.py {slug}`
3. **解析 JSON 输出**：提取 `number`, `title_cn`, `description`, `constraints`
4. **缺失项确认**：若脚本失败或返回空字段，一次性向用户确认缺少的信息，不要逐条追问
