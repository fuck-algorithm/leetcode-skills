...
1. 题号、中文标题、slug、题目链接（leetcode.cn）
2. 当前仓库 URL（通过 `git remote` 获取）
3. 至少一种可运行算法代码（Java/Python/Go/JavaScript 任一）
4. 若有多解法：每种解法的代码、复杂度、核心思路
5. 输入约束（用于校验与随机生成）
6. 演示数据结构类型（树/图/数组/链表/哈希/栈/队列等）
...
1. **需求补齐**：补全输入信息清单，明确当前阶段（M0-M4）。
2. **项目骨架初始化**：按模块化结构搭建页面、特性、服务、类型、样式。
3. **TDD 测试先行（新增）**：
   - 每新增一个 hook/component/feature，先写对应单元测试
   - 每新增一种解法的数据/动画映射，先补 `solutionRegistry.test.ts` 断言
   - 所有测试跑红（预期失败）后再开始实现
   - 运行 `npm run test:unit`，确保全部变绿后才进入下一步
4. **核心交互实现**：完成头部、输入栏、代码面板、可视化画布、控制面板。
5. **多解法隔离实现**（如适用）：每种解法独立步骤、独立代码映射、独立画布上下文。
6. **工程化与部署**：lint/type-check/build + GitHub Pages Action。
7. **Playwright 自测验收**：按验收矩阵执行并留存证据。
8. **逐条核对并出报告**：按 AC 与 Rubric 生成验收结论。
